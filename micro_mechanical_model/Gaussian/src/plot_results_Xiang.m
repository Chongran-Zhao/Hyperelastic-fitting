function plot_results_Xiang(paras, F_UT_list, P_UT_list)

P_UT_fit = get_P_ij_list(1, 1, paras, F_UT_list);
UT_lambda = zeros(length(P_UT_list), 1);
UT_lambda(:) = F_UT_list(1,1,:);

figure;

hold on;
plot(UT_lambda, P_UT_fit, 'Color', 'k', 'LineWidth', 2.5, 'LineStyle', '-');
plot(UT_lambda, P_UT_list, 'o', 'MarkerFaceColor', 'k', 'MarkerSize', 12); 
hold off;
l = legend('UT-fitting' );
