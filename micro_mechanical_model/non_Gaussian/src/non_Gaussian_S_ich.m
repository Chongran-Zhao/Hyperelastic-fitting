...
temp_2 = @(theta, phi) beta(theta, phi) * sin(theta) * sin(theta) * cos(phi) * cos(phi);
temp_3 = @(theta, phi) beta(theta, phi) * sin(theta) * sin(theta) * sin(phi) * sin(phi);

% 使用高斯积分计算
S_tilde_1 = mu * sqrt(N) / tilde_lambda_1 * Lebedev_quadrature(temp_1);
S_tilde_2 = mu * sqrt(N) / tilde_lambda_2 * Lebedev_quadrature(temp_2);
S_tilde_3 = mu * sqrt(N) / tilde_lambda_3 * Lebedev_quadrature(temp_3);
