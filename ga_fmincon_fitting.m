...
objectiveFunction = @(x) objective1(x, UT_strain, UT_stress, ET_strain, ET_stress, PS_strain, PS_stress, UT, ET, PS);
...
disp(['First Residual norm = ' num2str(resnorm)]);

objectiveFunction = @(x) objective2(x, UT_strain, UT_stress, ET_strain, ET_stress, PS_strain, PS_stress, UT, ET, PS);
...
paras = fmincon(objectiveFunction, [paras, 1/3, 1/3, 1/3], [], [], [], [], lb, ub, nonlcon, options);
resnorm = res(paras, UT_strain, UT_stress, ET_strain, ET_stress, PS_strain, PS_stress, UT, ET, PS);
disp(['Final Residual norm = ' num2str(resnorm)]);

end

% Objective function
function res = objective1(x, UT_strain, UT_stress, ET_strain, ET_stress, PS_strain, PS_stress, UT, ET, PS)
...
function f = objective2(x, UT_strain, UT_stress, ET_strain, ET_stress, PS_strain, PS_stress, UT, ET, PS)
    paras = x(1:end-3);
    weights = x(end-2:end);
...
function [lb, ub, nvars, UT, ET, PS] = Ogden4_Model_Init()
lb = [-Inf, -Inf, 0, 0];
ub = [0, 0, Inf, Inf];
nvars = 4;
UT = @(x, xdata) x(1) * ( xdata .^ (x(2) - 1.0) - xdata .^ (-0.5 * x(2) - 1.0) ) ...
